...
import { updateDiscussionCollectionsSyncjob } from '../../../../database/queries'
import { resolveTeamAndCheckMembership } from '#crouton/team-auth'
import type { DiscussionCollectionsSyncjob } from '../../../../../types'

export default defineEventHandler(async (event) => {
  const { syncjobId } = getRouterParams(event)
  const { team, user } = await resolveTeamAndCheckMembership(event)

  const body = await readBody<Partial<DiscussionCollectionsSyncjob>>(event)

  return await updateDiscussionCollectionsSyncjob(syncjobId, team.id, user.id, {
