...
import { updateDiscussionCollectionsTask } from '../../../../database/queries'
import { resolveTeamAndCheckMembership } from '#crouton/team-auth'
import type { DiscussionCollectionsTask } from '../../../../../types'

export default defineEventHandler(async (event) => {
  const { taskId } = getRouterParams(event)
  const { team, user } = await resolveTeamAndCheckMembership(event)

  const body = await readBody<Partial<DiscussionCollectionsTask>>(event)

  return await updateDiscussionCollectionsTask(taskId, team.id, user.id, {
