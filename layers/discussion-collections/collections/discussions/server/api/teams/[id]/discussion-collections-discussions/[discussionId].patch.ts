...
import { updateDiscussionCollectionsDiscussion } from '../../../../database/queries'
import { resolveTeamAndCheckMembership } from '#crouton/team-auth'
import type { DiscussionCollectionsDiscussion } from '../../../../../types'

export default defineEventHandler(async (event) => {
  const { discussionId } = getRouterParams(event)
  const { team, user } = await resolveTeamAndCheckMembership(event)

  const body = await readBody<Partial<DiscussionCollectionsDiscussion>>(event)

  return await updateDiscussionCollectionsDiscussion(discussionId, team.id, user.id, {
